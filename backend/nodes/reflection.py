...
async def reflect(claim_idea: str, claim_type: str, child_results: list[dict], passages: list[dict], loop_count: int, analysis_level: str = "l2") -> tuple[dict, dict]:
    ...
    print(f"    \033[34m[{label}]\033[0m loop={loop_count}, {len(passages)} passages, {len(context_json)} chars")
    ...
    if raw_response["parsed"] is None:
        print(f"    \033[34m[{label}]\033[0m \033[31mParsing failed — defaulting to sufficient=True\033[0m")
        result = {"sufficient": True, "follow_up_queries": []}
    else:
        parsed = raw_response["parsed"]
        result = {
            "sufficient": parsed.sufficient,
            "follow_up_queries": parsed.follow_up_queries[:2],
        }
        if parsed.sufficient:
            print(f"    \033[34m[{label}]\033[0m \033[32mSufficient — proceeding to synthesizer\033[0m")
        else:
            print(f"    \033[34m[{label}]\033[0m \033[33mInsufficient\033[0m")
            print(f"    \033[34m[{label}]\033[0m Follow-up queries: {parsed.follow_up_queries}")

    metrics = {
        "duration": duration,
        "passes": [
            {
                "agent": f"reflection_{analysis_level}",
