...
# These functions are used for file-based transcription (not streaming).
# They are exported and used by other modules (e.g., instagram.py).

async def upload_to_assemblyai(file_path: str) -> str:
    """Uploads an audio file to AssemblyAI and returns the upload_url pointing to the file on AssemblyAI's API."""
